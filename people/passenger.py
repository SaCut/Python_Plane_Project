...
from people.people import Person # Dunni's vs code doesn't recognise people as a package...
...
class Passenger(Person):
    ...
    def __str__(self):
        return f"{self.oid}. {self.first_name} {self.last_name} {self.age} {self.ticket_number} {self.passport_number}"

    def make_from_db(self, oid, first_name, last_name, age, ticket_number, passport_no):
        super().init_person_data(oid, first_name, last_name, age, ticket_number)
    ...
    def __save_and_regenerate_with_id(self, db_wrapper):
        ...
        db_wrapper.cursor.execute(
            ...
            + f"VALUES ('{first_name}', '{last_name}', {age}, '{ticket_number}', '{passport_number}');")

        db_wrapper.connection.commit()

        # Update the dictionary so that this is always correct
        psg = Passenger().make_from_db(self.get_max_id(db_wrapper), first_name, last_name, age, ticket_number, passport_number)
    ...
    def make_manual(self, first_name, last_name, age, ticket_no, passport_no, db_wrapper):
        # make a place holder passenger
        self.make_from_db(None, first_name, last_name, age, ticket_no, passport_no)
